 ...
 *
 * **Выключена с 26.08.2026** (`conversation_task_settings.is_enabled = false`). Она была
 * нужна, пока переписку было видно только внутри карточки: без задачи входящее сообщение
 * не всплывало нигде. С появлением раздела «Переписки» — со списком диалогов, счётчиком
 * непрочитанного в меню и отбором «непрочитанные» — она стала дублировать тот же сигнал,
 * только грубее: гаснет не когда прочитали, а когда нажали «выполнено».
 *
 * Код оставлен: выключатель для того и был предусмотрен, и вернуть всё — одна строка
 * `update`. Ниже описано, как оно работает, если включить обратно.
 *
