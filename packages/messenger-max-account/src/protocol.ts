...
export const MaxOpcode = {
  ...
  notifyMessage: 128,
  /**
   * Привязка нового устройства по QR — штатный способ MAX добавить себе клиента.
   *
   * Отличается от входа по коду тем, кто даёт разрешение: там его даёт владелец номера,
   * доказавший это кодом из SMS, здесь — владелец уже вошедшего устройства, нажимающий
   * «подтвердить» в приложении. Для нас это важно по-житейски: вход по коду их антифрод
   * от нашего клиента не принимает вовсе, отвечая требованием капчи.
   */
  getQr: 288,
  getQrStatus: 289,
  loginByQr: 291
