...
async function handleMessage(
  ...
  if (text === "" || text.startsWith("/")) {
    // Команды у MAX нет отдельного типа: `/start` приходит обычным текстом, а сам запуск
    // бота — типом `bot_started`. Повторно проводить человека по началу сценария не нужно.
    return;
  }
