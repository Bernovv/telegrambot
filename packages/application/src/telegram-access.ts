...
export interface TelegramPhoneStatusRepository {
  ...
  findPhoneStatus(externalUserId: string): Promise<TelegramPhoneStatus | null>;
}
...
export class CheckTelegramPhoneAccessService {
  ...
  async execute(query: { readonly externalUserId: string }): Promise<TelegramPhoneAccessResult> {
    const status = await this.repository.findPhoneStatus(query.externalUserId);
