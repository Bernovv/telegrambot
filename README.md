...
- версии рассылок с одним изменяемым черновиком и неизменяемой опубликованной историей:
  содержимое ограничено текстом и HTTPS-кнопками, schema v2 поддерживает четыре проверяемые
  переменные получателя с запасным обращением, версия привязана к точному готовому снимку
...
- операторы могут вручную приостанавливать, продолжать и окончательно отменять кампании на
  странице `/broadcasts`; команды защищены версией агрегата, требуют причину и атомарно создают
  аудит и событие в транзакционной очереди;
- тестовая отправка фиксирует точный снимок текущей сохранённой версии и активного получателя
  Telegram, асинхронно обрабатывается worker-ом, показывает историю результата в `/broadcasts`
  и не повторяет сообщение автоматически при неопределённом внешнем результате;
- массовые и тестовые доставки фиксируют контекст имени получателя до отправки; worker подставляет
  только разрешённые переменные schema v2, использует настроенное запасное обращение, а
  опубликованные версии v1 продолжает отправлять буквально;
- schema v3 рассылок фиксирует одно фото по абсолютному HTTPS-адресу; worker передаёт адрес
  Telegram напрямую, отправляет текст как подпись с проверкой лимита после персонализации и
  сохраняет HTTPS-кнопки как для массовой, так и для тестовой доставки;
- страница `/data-operations` принимает ограниченный CSV UTF-8 пользователей в staging:
  строгий парсинг, нормализация Telegram ID, username, E.164 phone и CRM ID, checksum-дедупликация
  batch, статусы строк, append-only хранение и audit без персональных данных; затем создаётся
  неизменяемый снимок точных, возможных и конфликтных совпадений; неоднозначные строки получают
  версионируемые решения администратора с optimistic locking, а все строки доступны курсорными
  страницами, но пользователи на этом шаге не изменяются;
...
- финализация решений, проверка актуальности снимка и chunked merge импорта, экспорт с отдельными правами,
  дедупликация пользователей и безопасная выдача файлов;
