...
## Срез ручного управления рассылкой

- Добавлены отдельные команды паузы, возобновления и окончательной отмены с обязательной причиной.
- PostgreSQL блокирует агрегат, проверяет `lockVersion` и допустимость перехода до изменения.
- Состояние, append-only audit и события `BroadcastPaused`, `BroadcastResumed` или
  `BroadcastCancelled` фиксируются атомарно.
- BFF пропускает только три точных маршрута, а `/broadcasts` показывает доступные для текущего
  состояния кнопки.
- Уже арендованные отправки могут сохранить подтверждённый результат после команды; новые записи
  после паузы или отмены не захватываются.
- Тестовая отправка реализована следующим срезом.

## Срез тестовой отправки рассылки

- Команда с `broadcasts.send`, актуальной версией агрегата и обязательной причиной фиксирует
  сохранённый черновик либо последнюю опубликованную версию.
- PostgreSQL проверяет активную Telegram identity и атомарно создаёт защищённую очередь,
  append-only audit и `BroadcastTestSendRequested`.
- Worker отправляет тест тем же grammY-адаптером, но без автоматических повторов, чтобы не
  создавать дубликаты после неопределённого ответа Telegram.
- Подтверждённые успехи, ошибки и истёкшие аренды фиксируются как `sent`, `failed` и `uncertain`
  с отдельными outbox-событиями.
- `/broadcasts` позволяет указать Telegram ID, поставить тест в очередь и автоматически обновляет
  историю последних результатов.
- Персонализация реализована следующим срезом.

## Срез персонализации рассылок

- Новые черновики используют schema v2 с четырьмя разрешёнными текстовыми переменными и
  обязательным запасным обращением.
- Schema v1 сохраняет буквальное поведение и не интерпретирует существующие фигурные скобки.
- PostgreSQL фиксирует контекст имени и Telegram username при подготовке массовой либо постановке
  тестовой доставки в очередь; повторные попытки не читают изменившийся профиль.
- Чистый application renderer подставляет только разрешённые значения и отклоняет результат,
  превышающий лимит Telegram.
- Редактор `/broadcasts` содержит выбор переменной, вставку в позицию курсора и настройку
  запасного обращения.

## Срез фото в рассылках

- Новые черновики используют schema v3 и могут фиксировать один абсолютный HTTPS URL фото.
- Текст фото ограничен как подпись: 1000 символов до подстановки и 1024 после персонализации.
- grammY-адаптер выбирает `sendPhoto`, передаёт URL Telegram напрямую и сохраняет прежние
  HTTPS-кнопки.
- Массовая и тестовая очереди используют один неизменяемый payload и общий классификатор ошибок.
- `/broadcasts` позволяет включить фото и сохранить его URL; тестовая отправка проверяет
  фактическую доступность до запуска кампании.
## Срез staging CSV-импорта пользователей

- Добавлен ограниченный CSV UTF-8 upload до 512 КиБ и 5000 строк через отдельный BFF allowlist.
- `csv-parse` выполняет строгий разбор с определением запятой, точки с запятой или табуляции.
- Application service нормализует Telegram ID, username, E.164 phone, CRM ID и имя, отклоняет
  опасные формулы и помечает дубликаты внутри файла.
- PostgreSQL хранит append-only batch и нормализованные строки; SHA-256 исходных байтов защищает
  от повторного создания batch при гонке.
- Audit не содержит телефон, имя или нормализованный payload.
- `/data-operations` показывает счётчики и первые 200 строк, но не предоставляет merge.

## Срез сопоставления CSV-импорта пользователей

- Добавлен immutable analysis: один идемпотентный снимок результатов на staging batch.
- Точными признаками являются Telegram ID, подтверждённый телефон и CRM ID; единственный текущий
  username или импортированный телефон создаёт только `POSSIBLE_MATCH`.
- Противоречащие идентификаторы и другой защищённый телефон дают `CONFLICT`; автоматического merge
  нет.
- Результаты различают новые строки, точные совпадения без изменений, дополнение пустых полей,
  возможные совпадения, конфликты, ошибки и дубликаты.
- Добавлен нормализованный справочник внешних CRM identities и audit без PII.
- `/data-operations` выполняет staging и анализ одним действием и показывает первые 200
  сопоставленных строк.

## Срез решений по строкам CSV-импорта

- Для `POSSIBLE_MATCH` и `CONFLICT` добавлены явные решения: создать отдельного пользователя,
  дополнить безопасные поля выбранного кандидата или пропустить строку.
- Создание отдельного пользователя запрещено для конфликта и совпадения по импортированному
  телефону; кандидат merge должен присутствовать в неизменяемом снимке.
- Решения версионируются append-only, защищены optimistic locking и атомарно создают audit без PII.
- `/data-operations` позволяет исправлять решения и курсорно просматривать все строки анализа
  страницами до 200 записей.
- Этот срез не изменяет пользователей.
- Следующий срез: финализация решений, проверка актуальности снимка и возобновляемый chunked merge.
